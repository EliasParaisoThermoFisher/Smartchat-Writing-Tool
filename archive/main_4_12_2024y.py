...
st.success(f"** Use this tool to rewrite any given text in the selected format! Select a button to process your input.**")


# begin: function definitions, must be at the top before the code that calls them

# function: (re)set message history
def set_message_history():
    st.session_state['messages'] = [SystemMessage(content="You are a helpful writing assistant.")]
...
def handle_submit(user_input):
    ...
    with st.spinner("Thinking..."):
        ...
        temp_messages = [SystemMessage(content='You are a helpful assistant.'), HumanMessage(content=get_rewrite_prompt(user_input))]
